...
- **Никакой телеметрии. Безопасно для корпоративной среды.** Никакой аналитики использования, никаких отчётов о сбоях, никаких phone-home. Единственный исходящий трафик — это вызовы Copilot API, которые делают ваши агенты, — можно безопасно развёртывать в пределах корпоративного периметра.
...
- Последняя сборка: [GitHub Releases](https://github.com/haya3/FleetKanban/releases/latest) → `com.fleetkanban.FleetKanban-win-Setup.exe`
...
- **Собирает ли FleetKanban телеметрию?** Нет. Никакой аналитики использования, никаких отчётов о сбоях и никакого phone-home-эндпоинта. Единственный исходящий трафик приложения — это вызовы Copilot API, которые агент делает во время выполнения задач (так же, как любой другой Copilot-клиент), плюс проверка версии, которую выполняет встроенный промпт обновления, обращаясь к GitHub Releases. Это делает FleetKanban безопасным для развёртывания в корпоративной среде — сочетайте его с подпиской Copilot Business или Enterprise, и ваш код останется в пределах корпоративного периметра.
