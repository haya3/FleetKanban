...
// the opt-in `initialize_if_empty` flag used by the UI's "このフォルダを
// git リポジトリ化しますか？" prompt.
...
// "初期コミットを作成" action before the user hits CreateTask.
...
// CreateTaskInput is the payload from the frontend's "+ 新規タスク" form.
...
// or StatusFailed — i.e. the "再実行" flow from the Kanban's 完了 column —
