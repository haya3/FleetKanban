...
func (p *Planner) Plan(ctx context.Context, t *task.Task) ([]*task.Subtask, string, error) {
	...
	session, err := p.client.CreateSession(ctx, &copilot.SessionConfig{
		...
		SystemMessage: &copilot.SystemMessageConfig{
			Mode: "replace",
			Content: "You are a task-decomposition planner. Break the goal down into a DAG of small, executable subtasks.\n" +
				"Do not modify any files (read-only).\n" +
				"Your entire output must be a single JSON block wrapped in " + plannerJSONTag + " markers. No extra explanation.",
		},
...
// "Goal:" block and inject new instructions.
func buildPlannerPrompt(t *task.Task) string {
	goal := SanitizeSingleLine(t.Goal)
	return fmt.Sprintf(`Decompose the following task into a DAG of executable subtasks.
...
Constraints:
  ...
  - Subtasks execute serially on the same worktree; parallelism in the DAG is a representational concept, not actual concurrency.
  - agent_role may be any descriptive name (e.g. planner, coder, tester, researcher, verifier).
  - **Using "reviewer" or "ai_reviewer" as an agent_role is forbidden**: review is
    performed by the dedicated AI Review phase in a separate session after execution,
    so do not duplicate it inside the plan. If a read-only verification subtask is
    needed, use a role such as "verifier" and restrict it to read-only analysis without
    generating changes.
