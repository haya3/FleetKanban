...
func TestResolvePath_ExistingFile(t *testing.T) {
	...
	// The resolved form should be absolute. Compare identity via os.SameFile
	// rather than string equality so short-8.3 ↔ long-name divergence on
	// Windows (e.g. TEMP pointing at C:\Users\RUNNER~1\...) does not make
	// the assertion flake.
	assert.True(t, filepath.IsAbs(resolved))
	expectedFi, err := os.Stat(f)
	require.NoError(t, err)
	actualFi, err := os.Stat(resolved)
	require.NoError(t, err)
	assert.True(t, os.SameFile(expectedFi, actualFi),
		"resolved path does not identify the same file: want=%q got=%q", f, resolved)
}
