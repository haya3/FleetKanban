...
func TestBuildPrompt_ReworkIncludesNoChangeGuidance(t *testing.T) {
	tk := &task.Task{
		WorktreePath:   `C:\wt`,
		Goal:           "do X",
		ReviewFeedback: "LICENSE をディレクトリ構成ツリーに追記してください",
	}
	p := BuildPrompt(tk)
	assert.Contains(t, p, "レビュー指摘")
	assert.Contains(t, p, "LICENSE をディレクトリ構成ツリーに追記してください")
...
func TestBuildSubtaskPrompt_ReworkIncludesNoChangeGuidanceAndSubtaskNote(t *testing.T) {
	tk := &task.Task{
		WorktreePath:   `C:\wt`,
		Goal:           "build README",
		ReviewFeedback: "LICENSE を追記して",
	}
	sub := &task.Subtask{Title: "write README", AgentRole: "coder"}
	p := BuildSubtaskPrompt(tk, sub)
	assert.Contains(t, p, "NO_CHANGE_NEEDED:")
	assert.Contains(t, p, "LICENSE を追記して")
	assert.Contains(t, p, "この subtask の範囲",
