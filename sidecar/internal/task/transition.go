...
//
// Phase 1 flow (AI Review が実装された後):
//
...
//	planning      ─▶ aborted         (user pressed ■ during planning)
...
//	in_progress   ─▶ aborted         (user pressed ■ during run)
...
var allowedTransitions = map[Status]map[Status]struct{}{
	...
	StatusFailed: {
		// The user can reopen a failed task from the Kanban's "完了" column
		// via the 再実行 pill. The sidecar clears ErrorCode / ErrorMessage
