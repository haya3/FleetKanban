...
String? _validateTokenInput(String token) {
  if (token.isEmpty) return 'トークンを入力してください';
  for (final p in _supportedTokenPrefixes) {
    if (token.startsWith(p)) return null;
  }
  return '対応していない形式です。Copilot SDK は gho_ / ghu_ / github_pat_ のみ受け付けます'
      '（classic ghp_ PAT は非対応）。';
}
...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  Future<void> _submitAddToken() async {
    ...
    if (label.isEmpty) {
      setState(() => _addError = 'ラベルを入力してください');
      return;
    }
        ...
        child: ListView(
          children: [
            ...
            const _Section(
              title: '登録リポジトリ',
              subtitle:
                  'ベースブランチを固定 (pin) しない場合は自動検出モード'
                  '（origin/HEAD → main → master → HEAD）で動作します。'
                  '特定のブランチから常に派生させたいリポジトリだけ pin してください。',
              child: _RegisteredRepositoryList(),
            ),
            const _Section(
              title: 'モデル（ステージ別）',
              subtitle:
                  'Phase 1 では Code ステージのモデルのみが実行時に使われます。'
                  'Plan / Review は将来のステージ分離に備えた保存用。',
              child: _StageModelPickers(),
            ),
            _Section(
              title: 'GitHub アクセストークン（ラベル付きで複数保存可）',
              subtitle:
                  'DPAPI でローカル暗号化。アクティブなラベルのトークンが Copilot SDK に渡されます。\n'
                  '対応形式: gho_ (OAuth ユーザー) / ghu_ (GitHub App ユーザー) / github_pat_ (fine-grained PAT)。'
                  'classic ghp_ PAT は Copilot SDK が非対応のため拒否されます。',
              child: Column(
                ...
                children: [
                  tokenList.when(
                    ...
                    error: (e, _) => CopyableErrorText(
                      text: '$e',
                      reportTitle: 'Settings / GitHub トークン一覧の取得',
                    ),
          ...
          children: [
            if (models.isEmpty)
              Padding(
                ...
                child: InfoBar(
                  title: const Text('モデル一覧を取得できません'),
                  content: const Text(
                    'Copilot にログインしていない、または SDK から ListModels を取得できませんでした。手入力した ID は保存されますが、実行時に ListModels でフォールバックされます。',
                  ),
...
class _StagePickerRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (savedValue != null && savedValue.isNotEmpty && !ids.contains(savedValue)) {
      ...
      children: [
        ...
        Expanded(
          child: saved.when(
            ...
            error: (e, _) => CopyableErrorText(
              text: '$e',
              reportTitle: 'Settings / モデル設定 ($label)',
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Tooltip(
      message: multiplier == 0
          ? 'Premium request を消費しません（プランに含まれる）'
          : 'リクエストごとに Premium request を ×$multiplier 消費します',
      child: Container(
...
class _RegisteredRepositoryList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return repositories.when(
      loading: () => const SizedBox(height: 40, child: ProgressBar()),
      error: (e, _) =>
          CopyableErrorText(text: '$e', reportTitle: 'Settings / 登録リポジトリの取得'),
      data: (repos) {
        if (repos.isEmpty) {
          return const Text('登録されたリポジトリはまだありません。');
        }
        return Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
        ...
        children: [
          ...
          if (!hasCommits)
            Padding(
              ...
              child: InfoBar(
                title: const Text('このリポジトリにはコミットがありません'),
                content: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text(
                      '`git init` 直後の状態です。ベースブランチを解決できないため、'
                      'このままではタスクを作成できません。'
                      'FleetKanban 名義の空コミットを作成して初期化できます。',
                    ),
          ...
          if (_error != null)
            Padding(
              padding: const EdgeInsets.only(top: 6),
              child: ErrorInfoBar(title: 'ベースブランチ設定に失敗', message: _error!),
            ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = auth.when(
      data: (a) => a.authenticated
          ? 'ログイン中: ${a.user}'
          : (a.message.isNotEmpty ? '未認証 — ${a.message}' : '未認証'),
      loading: () => '確認中…',
      error: (e, _) => '状態の取得に失敗: $e',
    );
        ...
        children: [
          ...
          if (_actionError != null)
            Padding(
              padding: const EdgeInsets.only(bottom: 8),
              child: ErrorInfoBar(
                title: _actionErrorTitle ?? '操作に失敗しました',
            ...
            children: [
              ...
              Button(
                onPressed: disabled
                    ? null
                    : () =>
                          _run(_AuthBusy.recheck, '認証状態の再確認に失敗しました', () async {
                            ref.invalidate(copilotAuthProvider);
          ...
          const SizedBox(height: 8),
          Text(
            'サインインは Device Flow。ボタンを押すと自動でブラウザが開き、'
            'コードがあらかじめ入力された状態で承認画面が表示されます。'
            'サインアウトだけは Copilot CLI に headless 手段が無いため、'
            '短命なターミナルで /logout を打つフローを残しています。',
