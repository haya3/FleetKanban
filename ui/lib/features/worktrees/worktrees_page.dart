      ...
      content: entries.when(
        ...
        data: (list) {
          if (list.isEmpty) {
            return const Center(
              child: Text(
                'No worktrees yet. Register a repository and create a task,\n'
                'and fleetkanban/<task-id> worktrees will appear here.',
          ...
          children: [
            ...
            Text(
              status == _RowStatus.missing
                  ? 'This directory was deleted externally. git worktree prune will also run.'
                  : 'Removes the worktree directory. Any uncommitted changes will be discarded.',
            ),
            if (isFleetkanban) ...[
  ...
  try {
    ...
    await displayInfoBar(
      ...
      builder: (_, close) => InfoBar(
        title: const Text('Failed to remove'),
        content: CopyableErrorText(
          text: '$e',
          reportTitle: 'Worktrees / remove failed',
        ),
        severity: InfoBarSeverity.error,
        action: IconButton(
