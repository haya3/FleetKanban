        ...
        data: (s) {
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                _Section(title: 'サマリ', child: _SummaryCard(s: s)),
                const SizedBox(height: 16),
                _Section(title: '所要時間', child: _DurationCard(s: s)),
                const SizedBox(height: 16),
                _Section(title: '30 日スループット', child: _ThroughputCard(s: s)),
                const SizedBox(height: 16),
                _Section(title: 'リワーク分布', child: _ReworkCard(s: s)),
                const SizedBox(height: 16),
                _Section(title: '失敗内訳', child: _FailureCard(s: s)),
                if (showRepoTable) ...[
                  const SizedBox(height: 16),
                  _Section(title: 'リポジトリ別', child: _RepositoryTable(s: s)),
                ],
              ],
            ),
          );
    ...
    return reposAsync.when(
      ...
      data: (repos) {
        final items = <ComboBoxItem<String?>>[
          const ComboBoxItem(value: null, child: Text('すべてのリポジトリ')),
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: _Metric(label: '総タスク', value: '${s.totalTasks}'),
            ),
            Expanded(
              child: _Metric(label: '進行中', value: '${s.activeTasks}'),
            ),
            Expanded(
              child: _Metric(
                label: '完了',
        ...
        Row(
          children: [
            Text('完了率', style: theme.typography.bodyStrong),
...
class _DurationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (s.completedSamples == 0) {
      return const Text('実行完了したタスクがまだありません。');
    }
    return Row(
      children: [
        Expanded(
...
class _FailureCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (buckets.isEmpty) {
      return const Text('失敗したタスクはありません。');
    }
    int maxCount = 0;
    for (final b in buckets) {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: _Metric(label: '完了 (30日)', value: '$totalDone'),
            ),
            Expanded(
              child: _Metric(label: '失敗 (30日)', value: '$totalFailed'),
            ),
            Expanded(
              child: _Metric(label: '最大 / 日', value: '$peak'),
            ),
          ],
        ),
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              _headerCell(theme, '合計'),
              _headerCell(theme, '完了'),
              _headerCell(theme, '失敗'),
              _headerCell(theme, '完了率'),
              _headerCell(theme, '平均所要'),
            ],
          ),
        ),
        const Divider(),
...
class _HistogramRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = barColor ?? theme.accentColor.defaultBrushFor(theme.brightness);
    return Padding(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Kanban 画面からタスクを作成・実行すると、ここに集計が表示されます。',
              style: theme.typography.caption?.copyWith(
                color: theme.resources.textFillColorTertiary,
              ),
            ),
