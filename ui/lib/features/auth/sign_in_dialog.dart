...
//      ("ブラウザを再度開く" button) while polling GetCopilotLoginSession
...
class _CopilotSignInDialogState extends ConsumerState<_CopilotSignInDialog> {
  ...
  Widget build(BuildContext context) {
    return ContentDialog(
      title: const Text('GitHub Copilot にサインイン'),
    ...
    switch (_phase) {
      ...
      case _Phase.waiting:
        ...
        return Column(
          ...
          children: [
            Text(
              '自動的にブラウザが開きました。GitHub のページで次のコードが'
              '既に入力されているはずです。もし入力されていなければ、'
              '下のコードをそのまま貼り付けて **Continue** → **Authorize** '
              'の順にクリックしてください。',
            ...
            InfoBar(
              title: const Text('Mobile App だけでは完了しません'),
              content: const Text(
                'GitHub の Device Flow は仕様上ブラウザでの承認が必須です。'
                'モバイル App は二要素認証プッシュが要求されたときだけ'
                '使われます。',
              ),
            ...
            Text(
              'ブラウザで Authorize を押すと自動的にこのダイアログが閉じます。',
        ...
        return Column(
          ...
          children: [
            ErrorInfoBar(
              ...
              message: _errorMessage ?? '不明なエラー',
            ),
            const SizedBox(height: 12),
            Text(
              'Copilot CLI のバンドル不整合か、ネットワーク到達性の問題が'
              '考えられます。再試行してもダメな場合は sidecar のログを'
              '確認してください。',
  ...
  List<Widget> _buildActions() {
    switch (_phase) {
      case _Phase.starting:
        return [Button(onPressed: _cancel, child: const Text('キャンセル'))];
      case _Phase.waiting:
        return [Button(onPressed: _cancel, child: const Text('キャンセル'))];
      case _Phase.failed:
        return [
          Button(onPressed: _cancel, child: const Text('閉じる')),
          FilledButton(onPressed: _start, child: const Text('再試行')),
        ];
      case _Phase.success:
        return const [];
    }
