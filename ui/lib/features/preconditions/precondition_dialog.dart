...
// "Later" dismiss. The dialog is re-shown on next launch if the user
    ...
    try {
      ...
      setState(() {
        ...
        _lastError = resp.error.isNotEmpty
            ? resp.error
            : 'Install finished but pwsh could not be detected.';
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        ...
        child: Column(
          ...
          children: [
            ...
            if (item.autoInstallable) ...[
              ...
              Text(
                'Auto install: uses winget for a per-user install.\n'
                'No UAC prompt; takes roughly 1–3 minutes.',
                ...
                content: Column(
                  ...
                  children: [
                    ...
                    if (item.manualCommand.isNotEmpty) ...[
                      const SizedBox(height: 8),
                      const Text('Manual install command:'),
