// ReworkDialog — modal shown when the reviewer chooses 再実行 from a Human
...
class _ReworkDialogState extends State<_ReworkDialog> {
  ...
  void _submit() {
    ...
    if (text.isEmpty) {
      setState(() => _error = 'フィードバックを入力してください');
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return ContentDialog(
      ...
      content: Column(
        ...
        children: [
          Text(
            'このタスクを queued に戻し、以下のフィードバックを次回の Copilot プロンプトの先頭に注入します。',
            style: theme.typography.caption,
          ),
          const SizedBox(height: 8),
          TextBox(
            controller: _controller,
            placeholder: '例: コミットメッセージを日本語にしてください',
            maxLines: 6,
            minLines: 4,
            autofocus: true,
      ...
      actions: [
        ...
        FilledButton(onPressed: _submit, child: const Text('再実行に回す')),
      ],
    );
  }
}
