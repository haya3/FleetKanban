...
//   3. Status chips    : e.g. "⚠ 中断", "復旧要" for failed/aborted cases
...
class TaskCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final title = lines.first.isEmpty ? '(ゴール未設定)' : lines.first;
...
List<Widget> _buildChips(pb.Task task) {
  ...
  switch (task.status) {
    case 'aborted':
      ...
      chips.add(const _Chip(label: '再開可', color: Color(0xFF8A5C00)));
    case 'failed':
      chips.add(
        _Chip(
          label: task.errorCode.isEmpty ? '失敗' : task.errorCode,
    ...
    for (var i = 0; i < stages.length; i++) {
      ...
      children.add(
        Container(
          ...
          child: Tooltip(
            message: model.isEmpty ? '$label: モデル未記録' : '$label: $model',
            child: Text(
              label,
              style: TextStyle(
                fontSize: 10,
...
class _StageModelRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget cell(String model) {
      ...
      return Expanded(
        child: Tooltip(
          message: model.isEmpty ? 'モデル未記録' : model,
          child: Text(
            shown,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    switch (task.status) {
      ...
      case 'aborted':
        ...
        // same three-way post-processing UI as Human Review, plus 再実行 to
...
// HumanReviewActions is the three-pill cluster (Keep / 再実行 / Discard) shown
...
// hover discoverability still surface it. The destructive 破棄 / Discard
// branch actions use icon-only mode so all three pills (Keep / 再実行 / 破棄)
...
// 再実行 re-queues the same goal via RunTask.
  ...
  Future<void> _confirmAndDiscard(BuildContext context, WidgetRef ref) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => ContentDialog(
        title: const Text('Discard しますか？'),
        content: Text(
          'worktree とブランチ ${task.branch} の両方を削除し、タスクは '
          'Cancelled になります。この操作は取り消せません。',
        ),
        actions: [
          Button(
  ...
  Future<void> _confirm(BuildContext context, WidgetRef ref) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => ContentDialog(
        ...
        content: Text(
          'ブランチ ${task.branch} を削除します（git branch -D）。'
          '未マージの差分があれば失われます。タスクの履歴は残ります。',
        ),
        actions: [
          Button(
            child: const Text('キャンセル'),
